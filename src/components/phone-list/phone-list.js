...
import { fetchPhones, phoneAddToCart, sum, getValue } from "../../actions";
      ...
      {phones.map((phone) => {
        return (
          <li key={phone.id}>
            <PhoneListItem
              phone={phone}
              plusOneItem={() => phoneAddToCart(phone.id)}
            />
          </li>
        );
      })}
...
class PhoneListContainer extends Component {
  ...
  render() {
    ...
    return (
      <>
        <PhoneList phones={phones} phoneAddToCart={onAddToCart} />
      </>
    );
  }
}
...
const mapDispatchToProps = (dispatch, { phonesStoreService }) => {
  return {
    ...
    sum: (n) => dispatch(sum(n)),
    getValue: (e) => dispatch(getValue(e)),
  };
};

export default withPhonesStoreService()(
  connect(mapStateToProps, mapDispatchToProps)(PhoneListContainer)
);
